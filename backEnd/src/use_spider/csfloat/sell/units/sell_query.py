...
class SellQuery:
    ...
    def get_latest_data(user_id):
        """获取指定用户最新一条 CSFloat 销售记录（ID 和订单时间）"""
        try:
    ...
    def get_earliest_not_end(user_id):
        """获取指定用户最早的一条未完成销售订单的时间"""
        try:
            records = CsFloatSellModel.find_all(
