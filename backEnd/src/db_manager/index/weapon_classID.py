...
class WeaponClassIDModel(BaseModel):
    ...
    def find_by_en_weapon_name(cls, en_weapon_name: str):
        """根据英文武器名（steam_hash_name）查询"""
        return cls.find_by_steam_hash_name(en_weapon_name)
    ...
    def get_icon_status_map(cls, steam_hash_names: List[str]) -> Dict[str, bool]:
        ...
        # 去重并过滤空值
        valid_names = [name for name in set(steam_hash_names) if name]
        if not valid_names:
            return {}
    ...
    def batch_update_steam_hash_name(cls, weapon_list: List[Dict[str, Any]]) -> int:
        """
        Steam专用：批量插入或更新steam_hash_name
            ...
            try:
                ...
                if existing_records:
                    ...
                    if affected_rows > 0:
                        success_count += 1
                        update_count += 1
                        print(f"✅ 更新Steam数据: steam_hash_name={data_hash_name}")
                else:
                    # 使用 INSERT 插入新记录
                    sql_insert = f'''INSERT INTO {cls.get_table_name()}
                    ...
                    if affected_rows > 0:
                        success_count += 1
                        insert_count += 1
                        print(f"✅ 插入新Steam数据: steam_hash_name={data_hash_name}")
