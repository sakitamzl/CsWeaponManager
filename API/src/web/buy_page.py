...
def getNowBuyingList(min, max):
    sql = f"SELECT ID, item_name, weapon_name, weapon_type, weapon_float,  float_range, price, `from`, order_time, status FROM `buy` LIMIT {min}, {max};"
    result = Date_base().select(sql)
    if result and len(result) == 2:
        flag, data = result
        if flag:
...
def getBuyDataByStatus(status, min, max):
    if status == 'all':
        ...
        sql = f"SELECT ID, item_name, weapon_name, weapon_type, weapon_float, float_range, price, `from`, order_time, status FROM `buy` WHERE status = '{status}' LIMIT {min}, {max};"
    result = Date_base().select(sql)
    if result and len(result) == 2:
        flag, data = result
        if flag:
