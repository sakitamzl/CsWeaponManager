...
def getLentData(min, max):
    sql = f"SELECT ID, weapon_name, item_name, weapon_float, float_range, price, lenter_name, status, last_status, `from`, lean_start_time, lean_end_time, total_Lease_Days, max_Lease_Days FROM `yyyp_lent` ORDER BY lean_start_time DESC LIMIT {min}, {max};"
    result = Date_base().select(sql)
    if result and len(result) == 2:
        flag, data = result
        if flag:
...
def getLentDataByStatus(status, min, max):
    if status == 'all':
        ...
        sql = f"SELECT ID, weapon_name, item_name, weapon_float, float_range, price, lenter_name, status, last_status, `from`, lean_start_time, lean_end_time, total_Lease_Days, max_Lease_Days FROM `yyyp_lent` WHERE status = '{status}' ORDER BY lean_start_time DESC LIMIT {min}, {max};"
    result = Date_base().select(sql)
    if result and len(result) == 2:
        flag, data = result
        if flag:
