...
class CSDB:
    ...
    def get_buy_data(limit, max_limit):
        sql = f"SELECT `weapon_name`, `weapon_type`, `item_name`, `weapon_float`, `float_range`, `price`, `from`, `order_time`,`status` FROM `{database_name}`.`Buy` where `from` = 'yyyp' limit {limit}, {max_limit}"
        flag, data = Date_base().select(sql)
        return jsonify(data), 200
    
    @CSDB_blueprint.route('/get_sell_data/<int:limit>/<int:max_limit>', methods=['post'])
    def get_sell_data(limit, max_limit):
        sql = f"SELECT `weapon_name`, `weapon_type`, `item_name`, `weapon_float`, `float_range`, `price`, `from`, `order_time` FROM `{database_name}`.`Sell` where `from` = 'yyyp' limit {limit}, {max_limit}"
        flag, data = Date_base().select(sql)
        return jsonify(data), 200
    ...
    def insert_webside_buydata():
        ...
        sql = (f"INSERT INTO `{database_name}`.`{data_from}_Buy` "
        ...
        if buy_number == 1 :
            sql = (f"INSERT INTO `{database_name}`.`Buy` "
            f"(`ID`, `weapon_name`, `weapon_type`, `item_name`, `weapon_float`, `float_range`, `price`,"
    ...
    def insert_webside_selldata():
        ...
        sql =  (f"INSERT INTO `{database_name}`.`{data_from}_Sell` "
                f"(`ID`, `weapon_name`, `weapon_type`, `item_name`, `weapon_float`, `float_range`, `price`,"
        ...
        if sell_number == 1:
            sql =  (f"INSERT INTO `{database_name}`.`Sell` "
                    f"(`ID`, `weapon_name`, `weapon_type`, `item_name`, `weapon_float`, `float_range`, `price`,"
                    f" `buyer_name`, `order_time`, `status`, `from`, `storage_time`, `steam_id`,"
                    f" `sell_number`, `err_number`, `price_all`)"
                    f" VALUES "
    ...
    def insert_webside_lentdata():
        ...
        sql = (f"INSERT INTO `BUFF`.`yyyp_Lent` (`ID`, `weapon_name`, `item_name`, `weapon_float`, `float_range`, `price`, `lenter_name`, "
