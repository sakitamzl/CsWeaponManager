  ...
  setup() {
    ...
    const loadOnSaleData = async () => {
      ...
      try {
        ...
        if (selectedTradeType.value === 'lease') {
        ...
        } else if (selectedTradeType.value === 'offer') {
          // 报价处理类型：调用获取我的出售订单API
          response = await axios.post(apiUrls.yyypGetMySellOrders(), {
            steamId: accountList.value.find(acc => acc.id === selectedAccount.value)?.steam_id || '',
            code: '1',
            subCode: '1-4',  // 获取待确认报价
            whetherDark: true
          ...
          if (response.data && response.data.success) {
            ...
            onSaleData.value = orderData.map(order => {
              return {
                id: order.offer_id,
                order_no: order.order_no,
                item_name: order.item_name,
                steam_hash_name: order.item_hash_name,
                sale_price: 0,  // 报价订单没有售价
                buy_price: null,
                weapon_float: null,  // 报价订单可能没有磨损值
                weapon_type: order.item_type,
                float_range: order.exterior,
                sticker: null,  // 报价订单暂不显示贴纸
                pendant: null,  // 报价订单暂不显示挂件
                rename: null,
                on_sale_time: order.end_countdown_desc,  // 显示倒计时描述
                platform: 'yyyp',
                trade_type: 'offer',
                account_id: selectedAccount.value,
                // 报价特有字段
            ...
            // 为每个订单启动倒计时
            onSaleData.value.forEach(item => {
              if (item.trade_type === 'offer' && item.remaining_seconds) {
                startCountdown(item)
              }
            })
          } else {
    ...
    const startCountdown = (item) => {
      if (item.trade_type !== 'offer' || !item.remaining_seconds) return
      ...
      const timer = setInterval(() => {
        const index = onSaleData.value.findIndex(i => i.id === item.id)
        if (index > -1 && onSaleData.value[index].remaining_seconds > 0) {
          onSaleData.value[index].remaining_seconds--
        } else {
          clearInterval(timer)
