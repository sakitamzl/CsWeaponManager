<template>
  ...
  >
    <div class="to-body">
      ...
      <!-- 求购余额 -->
      <div class="to-balance-block">
        <span class="to-balance-label">求购账户余额</span>
        <span class="to-balance-amount">¥{{ availableYuan.toFixed(2) }}</span>
      </div>

      <!-- 快捷选择 + 输入框 -->
      <div class="to-quick-row">
        <span class="to-section-label">快捷选择</span>
        ...
        <el-button size="small" type="warning" @click="setAll">全部转出</el-button>
      </div>
      ...
      <!-- 错误提示 -->
      <div v-if="errorMsg" class="to-error">{{ errorMsg }}</div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const handleConfirm = () => {
      ...
      if (amount < 10) {
        errorMsg.value = '最低转出金额为 ¥10'
        return
      }
      if (amount > props.availableYuan) {
        errorMsg.value = `不能超过求购余额 ¥${props.availableYuan.toFixed(2)}`
        return
      }
